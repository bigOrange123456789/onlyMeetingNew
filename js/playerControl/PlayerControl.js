function PlayerControl(camera){
    ...
    this.controller;
    this.init=function () {
        this.controller=new PlayerControl0(this.camera);
        function tool(){
...
function PlayerControl0(camera){
    ...
    this.up=function (step) {//相机的上方向是（0，1，0）
        var direction = new THREE.Vector3(
             this.camera.matrixWorld.elements[4]*step
            , this.camera.matrixWorld.elements[5]*step
            , this.camera.matrixWorld.elements[6]*step
        );
    ...
    this.forward=function (step) {//相机的初始方向是（0，0，-1）//对y旋转-90度后相机为水平方向camera.rotation.set(0,-Math.PI/2,0);
        var direction = new THREE.Vector3(
            - this.camera.matrixWorld.elements[8]*step
            ,- this.camera.matrixWorld.elements[9]*step
            ,- this.camera.matrixWorld.elements[10]*step
        );
        this.camera.position.set(
            this.camera.position.x+direction.x,
            this.camera.position.y+direction.y,
    ...
    this.left=function (step) {//相机的左方向是（-1，0，0）
        var direction = new THREE.Vector3(
            - this.camera.matrixWorld.elements[0]*step
            ,- this.camera.matrixWorld.elements[1]*step
            ,- this.camera.matrixWorld.elements[2]*step
        );
        this.camera.position.set(
            this.camera.position.x+direction.x,
            this.camera.position.y+direction.y,
